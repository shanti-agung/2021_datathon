...
_assumption_: for now, "life" sentence is treated as confinement of 90 years. Where 1 year = 365.2425 (as calculated in ec_sentence_period.Rmd)

```{r}
# create total_confinement_days and max_confinement_days
dockets_confinement <- dispositions %>% 
  filter(!is.na(sentence_type)) %>%
  mutate(max_period_days = if_else(max_period_parsed == "life", (90*365.2425),
                                   max_period_days)) %>% 
  group_by(docket_id) %>%
