...
```{r}
dockets_data <- dockets[,c("total_confinement_days", "max_confinement_days", "gender", "age",
                   "race","number_prior_dockets", "M", "M1", "M2", "M3",
...
#### Intercept model
```{r}
judge_total_intercept_model <- lmer(total_confinement_days ~ 1 + (1 | judge_id), dockets)
...
```{r}
judge_total_full_model <- lmer(total_confinement_days ~ 1 + gender + race + age +
                                 number_prior_dockets + M + M1 + M2 + M3 + `F` + F1 + F2 +
                                 F3 + S + S1 + S2 + S3 + IC + H1 + H2 + court_types_cp +
                                 court_types_mc + court_types_pac + year + month +
                                 (1 + gender + race + age +
                                 number_prior_dockets + M + M1 + M2 + M3 + `F` + F1 + F2 +
                                 F3 + S + S1 + S2 + S3 + IC + H1 + H2 | judge_id), dockets)
```
"Some predictor variables are on very different scales: consider rescaling"

```{r}
summary(dockets[,c("total_confinement_days", "max_confinement_days", "gender", "age",
                   "race","number_prior_dockets", "M", "M1", "M2", "M3",
                   "F", "F1", "F2", "F3", "S", "S1", "S2", "S3", "IC", "H1", "H2",
                   "court_types_cp", "court_types_mc", "court_types_pac", "year", "month",
                   "judge_id")])
