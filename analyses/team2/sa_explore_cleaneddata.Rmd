...
```
```{r}
judge_names_gt_10000 <- dispositions %>% 
  count(disposing_authority__first_name, disposing_authority__middle_name, 
...
```{r}
dispositions %>% 
  semi_join(judge_names_gt_10000) %>% 
  arrange(docket_id) %>% 
  ggplot(aes(x = disposing_authority__last_name, y = max_period_days)) +
  geom_boxplot() +
  coord_flip()
```
...
```{r}
dispositions_dockets %>% 
  filter(status_name %in% c("Active")) %>% 
  mutate(has_sentence = !is.na(sentence_type)) %>% 
  group_by(docket_id) %>% 
  summarise(num_sentence = sum(has_sentence)) %>% 
  ungroup() %>% 
...
dispositions_dockets %>% 
  ...
  mutate(has_sentence = !is.na(sentence_type)) %>% 
  group_by(docket_id) %>% 
  mutate(num_sentence = sum(has_sentence)) %>% 
  ungroup() %>% 
  filter(docket_id == 267625)
```
